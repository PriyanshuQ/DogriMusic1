...
function Player() {
  ...
  const onPrev = () => {
    if (currentSongIndex !== 0 && !shuffleOn) {
      ...
      dispatch(SetCurrentSong(randomIndex));
  ...
  return (
    <div className="bg-white absolute bottom-0 left-0 right-0 p-5">
      <div className="bg-gray-700 flex justify-between items-center border p-5 border-green-500 rounded ">
        <div className="flex items-center gap-3 w-96">
          <img
            className="h-18 w-16"
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcvR16q2dkth6IZWFTXcN-YNHJ80sZZmwZ7w&usqp=CAU"
            alt=""
          />
          <div>
          ...
          <div className="flex gap-3 items-center w-ful">
            <i
              className={`ri-shuffle-line text-xl ${
                shuffleOn && `text-orange-400 font-semibold`
              }`}
              onClick={() => {
